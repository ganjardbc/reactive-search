...
import {navbar} from '../payload';

const SearchFilters = ({ currentTopics, setTopics, visible }) => (
	<div className={`flex column filters-container ${!visible ? 'hidden' : ''}`}>
		<div className="child m10" style={{position: 'relative', height: 80}}>
			<label className="txt-site txt-9 txt-black txt-bold" style={{position: 'absolute', top: 7}}>
                Language
            </label>
			<MultiDropdownList
		...
		<div className="child m10">
			<RangeSlider
				...
				innerClass={{
					label: 'txt-site txt-10 txt-main txt-bold',
				}}
			/>
		</div>
		<div className="child m10">
		...
		<div style={{paddingTop: 30}}>
			{navbar && navbar.map((dt, index) => {
				return (
					<div key={index} style={{position: 'relative', height: 85}}>
						<label className="txt-site txt-9 txt-black txt-bold" style={{position: 'absolute', top: 7}}>
							{dt.label}
						</label>
						<SingleDropdownRange
